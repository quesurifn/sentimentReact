...
export default {
  ...
  getScopedEvents: function (events, namespace, childType, baseProps) {
    ...
    var parseEvent = function (eventReturn, eventKey) {
      ...
      var getMutationObject = function (key, childName) {
        ...
        var mutatedProps = mutation(_assign({}, mutationTargetProps, mutationTargetState), baseProps);
        var childState = _this2.state[childName] || {};
        var extendState = function (state) {
          return target === "parent" ? _extend(state[key], mutatedProps) : _extend(state[key], _defineProperty({}, target, mutatedProps));
        };
        return childName !== undefined && childName !== null ? _extend(_this2.state, _defineProperty({}, childName, _extend(childState, _defineProperty({}, key, extendState(childState))))) : _extend(_this2.state, _defineProperty({}, key, extendState(_this2.state)));
      };
    ...
    var parseEventReturn = function (eventReturn, eventKey) {
      return Array.isArray(eventReturn) ? eventReturn.reduce(function (memo, props) {
        memo = _merge({}, memo, parseEvent(props, eventKey));
        return memo;
  ...
  getPartialEvents: function (events, eventKey, childProps) {
    return events ? Object.keys(events).reduce(function (memo, eventName) {
      /* eslint max-params: 0 */
      memo[eventName] = _partial(events[eventName], _partial.placeholder, // evt will still be the first argument for event handlers
